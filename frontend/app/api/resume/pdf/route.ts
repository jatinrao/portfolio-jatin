...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const { slug, lang, theme } = parseResumeQuery(request.nextUrl.searchParams);

    logger.info("resume_pdf.request_received", { requestId, slug, lang, theme });
    ...
    return new NextResponse(new Uint8Array(pdfBuffer), {
      ...
      headers: {
        ...
        "Content-Disposition": `inline; filename="${slug}-${lang}-${theme}-resume.pdf"`,
...
//   return new NextResponse("PDF generation is currently disabled.", {
//     status: 503,
//     headers: {
